...
extern "C" bool awtrix_artnet_take_frame(uint32_t *rgb888, int max_pixels, int *out_pixels) __attribute__((weak));
...
extern "C" void awtrix_mqtt_subscribe(const char *topic_suffix) __attribute__((weak));
...
static void awtrix_draw_weekday_strip(Matrix &m, int16_t x, int16_t y,
    ...
    int todayIdx = c.startOnMonday ? ((ti->tm_wday + 6) % 7)   /* Mon..Sun */
                                   :  ti->tm_wday;             /* Sun..Sat */
...
void TimeApp(Matrix &m, UiState &, int16_t x, int16_t y, GifPlayer *) {
    ...
    switch (c.timeMode) {
    case 1: {  /* Calendar box: red header w/ weekday number, white body w/ date */
    ...
    case 5: {  /* BigTime — GIF backdrop unavailable until pack K; fallback to mode 0 */
        /* fall-through */
    }
    case 0:
    case 2:
...
void BatApp(Matrix &m, UiState &, int16_t x, int16_t y, GifPlayer *) {
    ...
    if (filled > 0) {
        ...
        uint32_t fillColor = (pct < 20) ? AwtrixColors::kBatteryLow
                          : (pct < 50)  ? AwtrixColors::kBatteryMid
...
void DisplayManager::applyAllSettings() {
    ...
    if (m_matrix && c.gamma > 0) m_matrix->applyGamma((float)c.gamma / 10.0f);
        ...
        case AwtrixCommandType::Button:
            if (awtrix_menu_active()) {
                switch (command.index) {
                    case 0: awtrix_menu_left(); break;
                    case 1: awtrix_menu_select_short(); break;
                    case 2: awtrix_menu_right(); break;
                    case 3: awtrix_menu_select_long(); break;
                    default: break;
                }
            } else {
                switch (command.index) {
                    case 0: leftButton(); break;
                    case 1: selectButton(); break;
                    case 2: rightButton(); break;
                    case 3: awtrix_menu_select_long(); break;
                    default: break;
                }
            }
...
void DisplayManager::loadCustomApps() {
    ...
    while ((de = readdir(dir)) != nullptr) {
        ...
        long sz = ftell(fp);
        fseek(fp, 0, SEEK_SET);
        if (sz <= 0 || sz > 16 * 1024) { fclose(fp); continue; }
...
bool DisplayManager::switchToApp(const char *json) {
    ...
    if (!name || !name->valuestring) { cJSON_Delete(doc); return false; }
...
void DisplayManager::updateAppVector(const char *json) {
    ...
    if (!arr || !cJSON_IsArray(arr)) { cJSON_Delete(doc); return; }
...
void DisplayManager::setPower(bool on) {
    ...
    SYS_TRANSITION(on ? awtrix::SystemState::Normal
...
bool DisplayManager::indicatorParser(uint8_t ind, const char *json) {
    ...
    if (!m_ui) { cJSON_Delete(doc); return false; }
    ...
    switch (ind) {
        case 1: m_ui->setIndicator1Color(col); m_ui->setIndicator1State(on); break;
        case 2: m_ui->setIndicator2Color(col); m_ui->setIndicator2State(on); break;
        case 3: m_ui->setIndicator3Color(col); m_ui->setIndicator3State(on); break;
    }
    cJSON_Delete(doc);
    return true;
}
...
bool DisplayManager::moodlight(const char *json) {
    ...
    if (col) {
        ...
        auto cl = [](double v){ if (v < 0) return 0; if (v > 255) return 255; return (int)v; };
...
bool DisplayManager::parseCustomPage(const char *name, const char *json, bool preventSave) {
    ...
    if (eff_s) {
        ...
        if (raw) { app.effectSettings = raw; cJSON_free(raw); }
    }
...
void DisplayManager::drawJPG(int16_t x, int16_t y, const uint8_t *data, uint32_t size) {
    (void)x; (void)y; (void)data; (void)size;
    static bool warned = false;
    if (!warned) { ESP_LOGW(TAG, "drawJPG called but no JPEG decoder linked; use drawBMP/RGB."); warned = true; }
}
...
void DisplayManager::drawBarChart(int16_t x, int16_t y, const int *data, uint8_t n,
    ...
    int barW = availW / n; if (barW < 1) barW = 1;
    int maxv = 1;
...
void DisplayManager::drawMenuIndicator(int cur, int total, uint32_t color) {
    ...
    int seg = w / total; if (seg < 2) seg = 2;
    for (int i = 0; i < total; i++) {
...
void DisplayManager::processDrawInstructions(int16_t x, int16_t y, const char *drawInstructions) {
    ...
    if (!doc || !cJSON_IsArray(doc)) { if (doc) cJSON_Delete(doc); return; }
    ...
    for (int i = 0; i < n; i++) {
        ...
        auto ai = [&](int k){ cJSON *v = cJSON_GetArrayItem(a, k); return v ? v->valueint : 0; };
        auto au = [&](int k){ cJSON *v = cJSON_GetArrayItem(a, k);
            if (!v) return (uint32_t)0;
            if (cJSON_IsNumber(v)) return (uint32_t)v->valuedouble;
            if (cJSON_IsString(v) && v->valuestring) {
                const char *s = v->valuestring; if (*s == '#') s++;
                return (uint32_t)strtoul(s, nullptr, 16);
            }
            return (uint32_t)0;
        };
...
bool DisplayManager::compileDrawInstructions(const char *json,
    ...
    for (int i = 0; i < n; i++) {
        ...
        int alen = cJSON_GetArraySize(a);
        auto ai = [&](int k){ cJSON *v = cJSON_GetArrayItem(a, k); return v ? (int16_t)v->valueint : (int16_t)0; };
        ...
        if ((!strcmp(o, "db") || !strcmp(o, "dfre")) && alen >= 5) {
            d.op = OPC_FILLRECT;
            d.a[0] = ai(0); d.a[1] = ai(1); d.a[2] = ai(2); d.a[3] = ai(3);
            ...
            d.op = OPC_PIXEL;
            d.a[0] = ai(0); d.a[1] = ai(1);
            d.color = au(cJSON_GetArrayItem(a, 2));
        } else if ((!strcmp(o, "dc") || !strcmp(o, "dci")) && alen >= 4) {
            d.op = OPC_CIRCLE;
            d.a[0] = ai(0); d.a[1] = ai(1); d.a[2] = ai(2);
...
void DisplayManager::show()  { if (m_matrix) { m_matrix->show(); update_display_snapshot_from_matrix(m_matrix); } }
...
void DisplayManager::selectButton() {
    ...
    extern void awtrix_menu_select_short() __attribute__((weak));
...
void DisplayManager::selectButtonLong() {
    extern void awtrix_menu_select_long() __attribute__((weak));
...
void DisplayManager::setNewSettings(const char *json) {
     ...
     * HTTP/MQTT/display/periphery readers never observe a half-applied batch. */
    {
        ...
        if ((v = cJSON_GetObjectItem(doc, "GAMMA"))&&cJSON_IsNumber(v))    c.gamma           = (uint8_t)(v->valuedouble * 10.0 + 0.5);
...
void DisplayManager::setCustomAppColors(uint32_t color) {
    for (auto &kv : APP_REGISTRY.customApps()) { kv.second.color = color; kv.second.hasCustomColor = true; }
}
...
void DisplayManager::sendAppLoop() {
    ...
    extern void awtrix_mqtt_publish_app_loop(const char *) __attribute__((weak));
...
void DisplayManager::startArtnet() {
    extern void awtrix_artnet_start() __attribute__((weak));
