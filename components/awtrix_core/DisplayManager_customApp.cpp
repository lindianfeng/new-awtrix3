...
static std::string custom_replace_placeholders(const std::string &in) {
    ...
    while (i < in.size()) {
        if (i + 1 < in.size() && in[i] == '{' && in[i+1] == '{') {
            ...
            else if (key == "CONFIG.brightness") { snprintf(numbuf, sizeof(numbuf), "%d", CONFIG.brightness); out += numbuf; }
            else if (key == "CONFIG.currentTemp") { snprintf(numbuf, sizeof(numbuf), "%.1f", CONFIG.currentTemp); out += numbuf; }
            else if (key == "CONFIG.currentHum")  { snprintf(numbuf, sizeof(numbuf), "%.0f", CONFIG.currentHum);  out += numbuf; }
            else if (key == "CONFIG.batteryPercent") { snprintf(numbuf, sizeof(numbuf), "%d", CONFIG.batteryPercent); out += numbuf; }
...
void renderCustomApp(Matrix &m, UiState &state, int16_t x, int16_t y, GifPlayer *) {
    ...
    if (app.progress >= 0 && app.progress <= 100) {
        ...
        if (filled > 0) m.fillRect(x, by, filled, 1, app.pColor ? app.pColor
