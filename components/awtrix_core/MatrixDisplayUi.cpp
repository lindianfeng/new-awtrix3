...
#include "awtrix_globals.h"   /* CONFIG.transEffect — now an explicit
                                 implementation dep after round 6 stopped
                                 leaking globals through the public header. */
...
void MatrixDisplayUi::tick()
{
    ...
    if (m_appCount > 0)
    {
        ...
        case UiState::FIXED:
            ...
            if (m_autoTrans&& m_state
            .
            ticksSinceLastStateSwitch >= m_ticksPerApp
            )
            m_state.appState = UiState::IN_TRANSITION;
            break;
        }
    ...
    const AwtrixRenderLayerInfo* visible[8] = {};
    const size_t count = RENDER_ENGINE.currentVisibleLayers(visible, sizeof(visible) / sizeof(visible[0]));
    for (size_t i = 0; i < count; ++i)
    {
        if (!visible[i]) continue;
        switch (visible[i]->layer)
        {
        case AwtrixRenderLayer::Background:
            if (m_bgEffect >= 0 && m_bgFunc) drawBackground();
            break;
        case AwtrixRenderLayer::App:
            if (m_appCount > 0) drawApp();
            break;
        case AwtrixRenderLayer::Notification:
            drawOverlayIndex(0);
            break;
        case AwtrixRenderLayer::WeatherOverlay:
        case AwtrixRenderLayer::Menu:
            /* Reserved for future dedicated layer renderers. */
            break;
        case AwtrixRenderLayer::SystemStatus:
            drawOverlayIndex(1);
            break;
        case AwtrixRenderLayer::Indicators:
            drawIndicators();
            break;
        }
    }
...
void MatrixDisplayUi::drawOverlays()
{
    ...
    for (uint8_t i = 0; i < m_overlayCount; i++) drawOverlayIndex(i);
}

void MatrixDisplayUi::drawOverlayIndex(uint8_t index)
{
    if (!m_overlayFuncs || index >= m_overlayCount || !m_overlayFuncs[index]) return;
    GifPlayer dummy;
    m_overlayFuncs[index](*m_matrix, m_state, &dummy);
}
...
void MatrixDisplayUi::doTransition()
{
    ...
    {
    case TX_FADE: txFade(m_matrix, m_appFuncs, m_state, curIdx, nextIdx, m_ticksPerTrans);
        break;
    case TX_ZOOM: txZoom(m_matrix, m_appFuncs, m_state, curIdx, nextIdx, m_ticksPerTrans);
        break;
    case TX_ROTATE: txRotate(m_matrix, m_appFuncs, m_state, curIdx, nextIdx, m_ticksPerTrans);
        break;
    case TX_PIXELATE: txPixelate(m_matrix, m_appFuncs, m_state, curIdx, nextIdx, m_ticksPerTrans);
        break;
    case TX_CURTAIN: txCurtain(m_matrix, m_appFuncs, m_state, curIdx, nextIdx, m_ticksPerTrans);
        break;
    case TX_RIPPLE: txRipple(m_matrix, m_appFuncs, m_state, curIdx, nextIdx, m_ticksPerTrans);
        break;
    case TX_BLINK: txBlink(m_matrix, m_appFuncs, m_state, curIdx, nextIdx, m_ticksPerTrans);
        break;
    case TX_RELOAD: txReload(m_matrix, m_appFuncs, m_state, curIdx, nextIdx, m_ticksPerTrans);
        break;
    case TX_CROSSFADE: txCrossfade(m_matrix, m_appFuncs, m_state, curIdx, nextIdx, m_ticksPerTrans);
        break;
    case TX_SLIDE:
    default:
