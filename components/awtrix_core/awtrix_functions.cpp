...
#include "awtrix_color_utils.h"   /* A1: shared HSV/lerp/fx helpers in awtrix_color:: */
#include "esp_timer.h"
#include <cJSON.h>
#include <cmath>
#include <cstring>
...
static void init_charwidth() {
    ...
    s_charWidth[' '] = 2; s_charWidth['!'] = 2; s_charWidth[0x27] = 2;
    s_charWidth['('] = 3; s_charWidth[')'] = 3; s_charWidth[','] = 3;
    s_charWidth['.'] = 2; s_charWidth[':'] = 2; s_charWidth[';'] = 3;
    s_charWidth['I'] = 2; s_charWidth['M'] = 6; s_charWidth['N'] = 5;
    s_charWidth['Q'] = 5; s_charWidth['W'] = 6; s_charWidth['i'] = 2;
    s_charWidth['l'] = 2; s_charWidth['|'] = 2;
...
uint32_t hsvToRgb(uint8_t h, uint8_t s, uint8_t v) {
    /* A1: forward to the shared kernel implementation in
     * <awtrix_color_utils.h> so the same hot-path lives in one
     * place. Inlined by the compiler — zero runtime overhead. */
    return awtrix_color::hsv_to_rgb(h, s, v);
}
...
uint8_t utf8ascii(uint8_t ascii) {
    if (ascii < 128) { s_utf8_c1 = 0; return ascii; }
...
uint32_t fadeColor(uint32_t color, uint32_t intervalMs) {
    ...
    float k = (phase < intervalMs) ? (float)phase / (float)intervalMs
