 ...
 * MenuManager textColors[] order). */
    ...
    static void inc_bri()
    {
        ...
        CONFIG.brightness = (CONFIG.brightness < 250) ? CONFIG.brightness + 5 : 255;
    }

    static void dec_bri()
    {
        AwtrixConfig::Guard g(CONFIG);
        CONFIG.brightness = (CONFIG.brightness > 5) ? CONFIG.brightness - 5 : 0;
    }
    ...
    static void tog_atrans()
    {
        AwtrixConfig::Guard g(CONFIG);
        CONFIG.auto_transition = !CONFIG.auto_transition;
    }

    static const char* show_atrans() { return fmt_bool(CONFIG.auto_transition); }

    /* ── T-SPEED (transition speed in ms) ───────────────────────── */
    static void inc_tspeed()
    {
        AwtrixConfig::Guard g(CONFIG);
        CONFIG.timePerTransition = (CONFIG.timePerTransition < 2000) ? CONFIG.timePerTransition + 100 : 2000;
    }

    static void dec_tspeed()
    {
        AwtrixConfig::Guard g(CONFIG);
        CONFIG.timePerTransition = (CONFIG.timePerTransition > 200) ? CONFIG.timePerTransition - 100 : 100;
    }

    static const char* show_tspeed() { return fmt_int((int)CONFIG.timePerTransition); }

    /* ── APPTIME (per-app dwell in s) ───────────────────────────── */
    static void inc_atime()
    {
        AwtrixConfig::Guard g(CONFIG);
        CONFIG.timePerApp += 500;
    }

    static void dec_atime()
    {
        AwtrixConfig::Guard g(CONFIG);
        if (CONFIG.timePerApp > 1000) CONFIG.timePerApp -= 500;
    }
    ...
    {
        AwtrixConfig::Guard g(CONFIG);
        CONFIG.startOnMonday = !CONFIG.startOnMonday;
    }
    ...
    {
        AwtrixConfig::Guard g(CONFIG);
        if (CONFIG.soundVolume < 30) CONFIG.soundVolume++;
    }

    static void dec_vol()
    {
        AwtrixConfig::Guard g(CONFIG);
        if (CONFIG.soundVolume > 0) CONFIG.soundVolume--;
    }

    static const char* show_vol() { return fmt_int(CONFIG.soundVolume); }

    /* ── SwBtn / Rotate (carried over from previous port) ───────── */
    static void tog_swap()
    {
        AwtrixConfig::Guard g(CONFIG);
        CONFIG.swap_buttons = !CONFIG.swap_buttons;
    }
    ...
    {
        AwtrixConfig::Guard g(CONFIG);
        CONFIG.rotate_screen = !CONFIG.rotate_screen;
    }
...
void awtrix_menu_enter(void)
{
    ...
    for (int i = 0; i < kTextColorCount; ++i)
        if (kTextColors[i] == CONFIG.textColor888)
        {
            s_colorIdx = i;
            break;
        }
    for (int i = 0; i < kTimeFormatCount; ++i)
        if (CONFIG.timeFormat == kTimeFormats[i])
        {
            s_timeFmtIdx = i;
            break;
        }
    for (int i = 0; i < kDateFormatCount; ++i)
        if (CONFIG.dateFormat == kDateFormats[i])
        {
            s_dateFmtIdx = i;
            break;
        }
