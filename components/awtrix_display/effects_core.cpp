...
void fx_TheaterChase(Matrix& m, int16_t x, int16_t y, EffectSettings* s)
{
    ...
    static int j = 0;
    if (_now() - last > (uint32_t)(100 - s->speed * 10))
    {
        last = _now();
        j++;
    }
...
void fx_LookingEyes(Matrix& m, int16_t x, int16_t y, EffectSettings* s)
{
    static uint32_t last = 0;
    static int dir = 0;
    if (_now() - last > (uint32_t)(800 - s->speed * 50))
    {
        last = _now();
        dir = std::rand() % 3 - 1;
    }
...
void fx_Checkerboard(Matrix& m, int16_t x, int16_t y, EffectSettings* s)
{
    static uint32_t last = 0;
    static int flip = 0;
    if (_now() - last > (uint32_t)(400 - s->speed * 30))
    {
        last = _now();
        flip ^= 1;
    }
...
void fx_Fade(Matrix& m, int16_t x, int16_t y, EffectSettings* s)
{
    ...
    static uint32_t last = 0;
    if (_now() - last > (uint32_t)(40 - s->speed * 3))
    {
        last = _now();
        hue++;
    }
...
void EffectOverlay(Matrix& m, int16_t x, int16_t y, int effect)
{
    ...
    int rainChance = (effect == OVERLAY_STORM || effect == OVERLAY_THUNDER)
                         ? 60
                         : (effect == OVERLAY_DRIZZLE ? 15 : 50);
    CRGB dropColor = (effect == OVERLAY_DRIZZLE)
                         ? hsv_to_rgb(160, 255, 230)
