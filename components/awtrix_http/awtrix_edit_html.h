...
/* ── P1-E: SPIFFS file editor (gzip-compressed HTML) ─────────────────
 * Verbatim port of lib/webserver/edit_htm.h from the original AWTRIX3
 * Arduino sources. ~39 KB gzip → ~120 KB inflated single-page editor
 * (syntax-highlighting + tabs + drag-drop upload), served at /files.
 *
 * The on-disk array is delivered as `Content-Encoding: gzip`; the browser
 * inflates on the fly. Use `sizeof(EDIT_HTML)` from C++ for the byte
 * count instead of a manual SIZE constant (avoids drift if we ever
 * re-export the upstream file).
 *
 * Same Arduino-isms removed as in awtrix_setup_html.h:
 *   - `PROGMEM` qualifier: not meaningful on IDF (transparent flash map)
 *   - `char[]` → `uint8_t[]` because the payload is raw binary.
 */
static const uint8_t EDIT_HTML[] = {
