...
static bool mqtt_accept_json_payload(const std::string &topic, const std::string &payload, size_t limit = kMaxMqttJsonPayload) {
...
static void process_message(const std::string &topic, const std::string &payload) {
    ...
    auto post = [&](AwtrixCommand command) {
    ...
    auto executeLocalParsed = [&](const char *suffix, const std::string &payloadText) {
        ...
        switch (command.type) {
            case AwtrixCommandType::R2D2: per.r2d2(command.payload.c_str()); break;
            case AwtrixCommandType::Rtttl: per.playRTTTL(command.payload.c_str()); break;
            case AwtrixCommandType::Sound: per.parseSound(command.payload.c_str()); break;
            case AwtrixCommandType::Reboot: esp_restart(); break;
            default: post(command); break;
        }
    };
