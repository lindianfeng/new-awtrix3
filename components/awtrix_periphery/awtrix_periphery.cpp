...
static void _button_event_cb(void* arg, void* usr_data)
{
    ...
    switch (evt)
    {
    case BUTTON_SINGLE_CLICK: peri.enqueueBtnEvent(idx, BTN_EVENT_PRESSED);
        break;
    case BUTTON_DOUBLE_CLICK: peri.enqueueBtnEvent(idx, BTN_EVENT_DOUBLE_PRESS);
        break;
    case BUTTON_LONG_PRESS_START: peri.enqueueBtnEvent(idx, BTN_EVENT_LONG_PRESS);
        break;
...
void PeripheryManager::tick()
{
    ...
    if (now - m_lastBatTempHum >= 10000)
    {
        ...
        if (cfg.sensorReading)
        {
            ...
            {
            case SENSOR_BME280:
                awtrix_i2c_read_bme280(&cfg.currentTemp, &cfg.currentHum);
                break;
            case SENSOR_BMP280:
                awtrix_i2c_read_bmp280(&cfg.currentTemp);
                break;
            case SENSOR_HTU21DF:
                awtrix_i2c_read_htu21df(&cfg.currentTemp, &cfg.currentHum);
                break;
            case SENSOR_SHT31:
                awtrix_i2c_read_sht31(&cfg.currentTemp, &cfg.currentHum);
                break;
            default: break;
            }
...
void PeripheryManager::playBootSound()
{
    ...
    else
    {
        ...
        awtrix_buzzer_tone(523);
        vTaskDelay(pdMS_TO_TICKS(150));
        awtrix_buzzer_tone(659);
        vTaskDelay(pdMS_TO_TICKS(150));
        awtrix_buzzer_tone(784);
        vTaskDelay(pdMS_TO_TICKS(150));
